...
import CourseSelectionSystem.urls
...
def load_course_data():
    ...
    for course_item in data.values():
        ...
        if len(id)>4:
            ...
            # Instructor.objects.get_or_create(instructor_id=course_item[])

def load_students_data():
    student_file = open('./test.csv','r')
    # test_set = set()
    for line in student_file:
        line = line.split(",")
        print(line)
        Student.objects.create(student_id=int(line[7]),student_name=line[8],student_password=line[7],student_major=line[5],student_dept_name=line[0])
        # length = len(test_set)
        # test_set.add(line[7])
        # if len(test_set) == length:
        #     print("!!!!!!!!!!!")
